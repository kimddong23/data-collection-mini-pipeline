...
- 작성자 : 신주용 (광주 3반)
- 작성일 : 2026-07-20
- 저장소 : https://github.com/kimddong23/data-collection-mini-pipeline

3개 공개 API 를 비동기로 동시 수집 → Pydantic v2 로 타입·범위 검증 →
검증 통과분을 CSV·Parquet 로 저장하고 읽기/쓰기 성능 비교.

## 코드 구조
```
data-collection-mini-pipeline/
├── pipeline/               # 소스 패키지
│   ├── __init__.py
...
├── tests/
│   └── test_models.py      # 스키마 검증 테스트 (pytest)
├── docs/                   # 제출 산출물 (실행 캡쳐 · 정리 PDF)
...
## 설명
- 관심사 분리 : 수집(collector) · 검증(models) · 저장(storage) · 진입점(main)
- 사용 API
...
python -m pipeline.main     # 파이프라인 실행
pytest -q                   # 테스트
ruff check .                # 스타일 검사
```
...
## 제출 문서 (실행결과 정리 PDF)
실행결과 캡쳐 + 코드 분석 본인 의견 + 추가 의견(개선사항·코드 품질) 정리.

![정리 1](docs/실행결과정리-1.png)
![정리 2](docs/실행결과정리-2.png)
![정리 3](docs/실행결과정리-3.png)
...
- 컴프리헨션 : 시간별 레코드 변환·필드 추출을 `zip` + 딕셔너리 컴프리헨션으로 처리
- 중복 제거 : CSV·Parquet 측정을 job 리스트 + 공용 측정 함수 하나로 통일
- 표준·기존 도구 우선 : pathlib·pandas 활용으로 새 코드 최소화
